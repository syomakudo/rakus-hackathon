...
<template>
  <div class="mx-auto my-5 px-4">
      ...
      <div class="mt-5" v-if="chatList.length !== 0">
        <ul>
          <li class="item mt-4" v-for="(chat, i) in chatList" :key="chat.id">
            {{ chat.text }}
            <button
              v-if="chat.userName === userName && isCancelMessage"
    ...
    <button type="button" class="button-normal button-exit" @click="onTest">
      test
    </button>
    ...
    <!-- 設定画面へ遷移 -->
    <router-link to="/chat/setting" class="link">
      <button type="button" class="button-normal button-exit">設定</button>
    </router-link>
  </div>
</template>
